...
Define_Module(AODVQoSRouting);

void AODVQoSRouting::initialize(int stage)
{
...
void AODVQoSRouting::handleMessage(cMessage *msg)
{
    if (!isOperational)
    {
    ...
    else
    {
        ...
        {
            case RREQ:
                handleRREQ(check_and_cast<AODVQoSRREQ *>(ctrlPacket), sourceAddr, arrivalPacketTTL);
                break;

            case RREP:
                handleRREP(check_and_cast<AODVQoSRREP *>(ctrlPacket), sourceAddr);
                break;

            case RERR:
                handleRERR(check_and_cast<AODVQoSRERR *>(ctrlPacket), sourceAddr);
                break;

            case RREPACK:
                handleRREPACK(check_and_cast<AODVQoSRREPACK *>(ctrlPacket), sourceAddr);
                break;
...
void AODVQoSRouting::recordUtilization(int packetSize, Path path)
{
    ...
    TransmissionOverHead overhead = { packetSize, simTime() };
    ...
    {
        std::vector<TransmissionOverHead> tmpVector;
        tmpVector.push_back(overhead);
        currentTransmissions[path] = tmpVector;
    }
...
INetfilter::IHook::Result AODVQoSRouting::ensureRouteForDatagram(INetworkDatagram * datagram)
{
    ...
    {
        recordUtilization(destAddr.isBroadcast() ? header->getTotalLengthField()+14:header->getTotalLengthField());
    }
    else if(header->getTransportProtocol()==IP_PROT_UDP)
    {
        if(sourceAddr.isUnspecified())
        {
            recordUtilization(header->getTotalLengthField()+14, Path(getSelfIPAddress(),destAddr));
        }
        else
        {
            recordUtilization(header->getTotalLengthField()+14, Path(sourceAddr,destAddr));
        }
    }
...
L3Address AODVQoSRouting::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void AODVQoSRouting::delayDatagram(INetworkDatagram *datagram)
{
...
void AODVQoSRouting::sendRREQ(AODVQoSRREQ *rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void AODVQoSRouting::sendRREP(AODVQoSRREP *rrep, const L3Address& destAddr, unsigned int timeToLive)
{
    ...
    AODVQoSRouteData *destRouteData = check_and_cast<AODVQoSRouteData *>(destRoute->getProtocolData());
...
AODVQoSRREQ *AODVQoSRouting::createRREQ(const L3Address& destAddr, double minAvailableBandwidth, double minAvailableSlotTime)
{
    ...
    if (lastKnownRoute && lastKnownRoute->getSource() == this)
    {
        ...
        AODVQoSRouteData *routeData = check_and_cast<AODVQoSRouteData *>(lastKnownRoute->getProtocolData());
...
AODVQoSRREP *AODVQoSRouting::createRREP(AODVQoSRREQ *rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
    AODVQoSRREP *rrep = new AODVQoSRREP("AODV-RREP");
    ...
    {    // intermediate node
        ...
        AODVQoSRouteData *destRouteData = check_and_cast<AODVQoSRouteData *>(destRoute->getProtocolData());
        AODVQoSRouteData *originatorRouteData = check_and_cast<AODVQoSRouteData *>(originatorRoute->getProtocolData());
...
AODVQoSRREP * AODVQoSRouting::createGratuitousRREP(AODVQoSRREQ * rreq, IRoute * originatorRoute)
{
    ...
    AODVQoSRouteData *routeData = check_and_cast<AODVQoSRouteData *>(originatorRoute->getProtocolData());
...
void AODVQoSRouting::handleRREP(AODVQoSRREP *rrep, const L3Address& sourceAddr)
{
    ...
    AODVQoSRouteData *destRouteData = nullptr;
    ...
    if (getSelfIPAddress() != rrep->getOriginatorAddr())
    {
        ...
        if (originatorRoute && originatorRoute->getSource() == this)
        {
            ...
            if (simTime() > rebootTime + deletePeriod || rebootTime == 0)
            {
                ...
                {
                    AODVQoSRREPACK *rrepACK = createRREPACK();
                ...
                {
                    AODVQoSRouteData *nextHopToDestRouteData = check_and_cast<AODVQoSRouteData *>(nextHopToDestRoute->getProtocolData());
                    nextHopToDestRouteData->addPrecursor(originatorRoute->getNextHopAsGeneric());
                }
                AODVQoSRREP *outgoingRREP = rrep->dup();
...
void AODVQoSRouting::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
    ...
    AODVQoSRouteData *routingData = check_and_cast<AODVQoSRouteData *>(route->getProtocolData());
...
void AODVQoSRouting::sendAODVPacket(AODVQoSControlPacket *packet, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void AODVQoSRouting::handleRREQ(AODVQoSRREQ *rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
    ...
    else
    {
        ...
        if (rreqSeqNum > routeSeqNum || (rreqSeqNum == routeSeqNum && newHopCount < routeHopCount) || rreq->getUnknownSeqNumFlag())
        {
    ...
    if (rreq->getDestAddr() == getSelfIPAddress())
    {
        ...
        // create RREP
        AODVQoSRREP *rrep = createRREP(rreq, destRoute, reverseRoute, sourceAddr);
    ...
    if (destRouteData && destRouteData->isActive() && destRouteData->hasValidDestNum() && destRouteData->getDestSeqNum() >= rreq->getDestSeqNum())
    {
        ...
        if (rreq->getGratuitousRREPFlag())
        {
            ...
            AODVQoSRREP *grrep = createGratuitousRREP(rreq, originatorRoute);
...
IRoute *AODVQoSRouting::createRoute(const L3Address& destAddr, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
    IRoute *newRoute = routingTable->createRoute();
    AODVQoSRouteData *newProtocolData = new AODVQoSRouteData();
...
void AODVQoSRouting::receiveSignal(cComponent *source, simsignal_t signalID, long value, cObject* details)
{
    RadioBehavior behavior;
    if (signalID == IRadio::receptionStateChangedSignal)
    {
        IRadio::ReceptionState radioMode = (IRadio::ReceptionState) value;

        switch (radioMode)
        {
            case IRadio::ReceptionState::RECEPTION_STATE_BUSY:
                behavior = RadioBehavior::BUSY;
                break;
            case IRadio::ReceptionState::RECEPTION_STATE_RECEIVING:
                behavior = RadioBehavior::BUSY;
                break;
            case IRadio::ReceptionState::RECEPTION_STATE_IDLE:
                behavior = RadioBehavior::FREE;
                break;
            case IRadio::ReceptionState::RECEPTION_STATE_UNDEFINED:
                behavior = RadioBehavior::UNDEFINED;
                break;
        }
    }
    else if (signalID == IRadio::transmissionStateChangedSignal)
    {
        IRadio::TransmissionState radioMode = (IRadio::TransmissionState) value;

        switch (radioMode)
        {
            case IRadio::TransmissionState::TRANSMISSION_STATE_TRANSMITTING:
                behavior = RadioBehavior::BUSY;
                break;
            case IRadio::TransmissionState::TRANSMISSION_STATE_IDLE:
                behavior = RadioBehavior::FREE;
                break;
            case IRadio::TransmissionState::TRANSMISSION_STATE_UNDEFINED:
                behavior = RadioBehavior::UNDEFINED;
                break;
        }
    }

    if (radioBehaviorTimeAllocation.size() == 0)
    {
    ...
    if (tmpTimeAllocation == radioBehaviorTimeAllocation.end())
    {
...
void AODVQoSRouting::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void AODVQoSRouting::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
    ...
    AODVQoSRouteData *unreachableRouteData = check_and_cast<AODVQoSRouteData *>(unreachableRoute->getProtocolData());
    ...
    for (int i = 0; i < routingTable->getNumRoutes(); i++)
    {
        ...
        AODVQoSRouteData *routeData = dynamic_cast<AODVQoSRouteData *>(route->getProtocolData());
    ...
    AODVQoSRERR *rerr = createRERR(unreachableNodes);
    rerrCount++;
...
AODVQoSRERR *AODVQoSRouting::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
    AODVQoSRERR *rerr = new AODVQoSRERR("AODV-RERR");
...
void AODVQoSRouting::handleRERR(AODVQoSRERR *rerr, const L3Address& sourceAddr)
{
    ...
    if (unreachableNeighbors.size() > 0 && (simTime() > rebootTime + deletePeriod || rebootTime == 0))
    {
        ...
        AODVQoSRERR *newRERR = createRERR(unreachableNeighbors);
...
bool AODVQoSRouting::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
...
void AODVQoSRouting::clearState()
{
...
void AODVQoSRouting::forwardRREP(AODVQoSRREP *rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void AODVQoSRouting::completeRouteDiscovery(const L3Address& target)
{
...
AODVQoSRREP * AODVQoSRouting::createHelloMessage()
{
    ...
    AODVQoSRREP *helloMessage = new AODVQoSRREP("AODV-HelloMsg");
...
void AODVQoSRouting::sendHelloMessagesIfNeeded()
{
    ...
    for (int i = 0; i < routingTable->getNumRoutes(); i++)
    {
        ...
        {
            AODVQoSRouteData *routeData = check_and_cast<AODVQoSRouteData *>(route->getProtocolData());
    ...
    if (hasActiveRoute && (lastBroadcastTime == 0 || simTime() - lastBroadcastTime > helloInterval))
    {
        ...
        AODVQoSRREP *helloMessage = createHelloMessage();
...
void AODVQoSRouting::handleHelloMessage(AODVQoSRREP *helloMessage)
{
    ...
    {
        AODVQoSRouteData *routeData = check_and_cast<AODVQoSRouteData *>(routeHelloOriginator->getProtocolData());
...
void AODVQoSRouting::expungeRoutes()
{
    for (int i = 0; i < routingTable->getNumRoutes(); i++)
    {
        IRoute *route = routingTable->getRoute(i);
...
void AODVQoSRouting::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result AODVQoSRouting::datagramForwardHook(INetworkDatagram *datagram, const InterfaceEntry *inputInterfaceEntry, const InterfaceEntry *& outputInterfaceEntry, L3Address& nextHopAddress)
{
    ...
    AODVQoSRouteData *routeDestData = routeDest ? dynamic_cast<AODVQoSRouteData *>(routeDest->getProtocolData()) : nullptr;
...
void AODVQoSRouting::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit)
    {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
...
AODVQoSRREPACK * AODVQoSRouting::createRREPACK()
{
    AODVQoSRREPACK *rrepACK = new AODVQoSRREPACK("AODV-RREPACK");
    rrepACK->setPacketType(RREPACK);
    rrepACK->setByteLength(2);
    return rrepACK;
}

void AODVQoSRouting::sendRREPACK(AODVQoSRREPACK *rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void AODVQoSRouting::handleRREPACK(AODVQoSRREPACK *rrepACK, const L3Address& neighborAddr)
{
...
void AODVQoSRouting::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void AODVQoSRouting::handleBlackListTimer()
{
