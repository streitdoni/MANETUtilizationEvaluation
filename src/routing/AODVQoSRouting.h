...
class INET_API AODVQoSRouting: public cSimpleModule,
        public ILifecycle,
...
protected:
    ...
    public:
        ...
        bool operator==(const RREQIdentifier& other) const {
            return this->originatorAddr == other.originatorAddr
    ...
    public:
        bool operator()(const RREQIdentifier& lhs,
                const RREQIdentifier& rhs) const {
...
protected:
    ...
    /* Route Discovery */
    void startRouteDiscovery(const L3Address& target, unsigned int timeToLive =
            0);
    ...
    void updateRoutingTable(IRoute *route, const L3Address& nextHop,
            unsigned int hopCount, bool hasValidDestNum,
            unsigned int destSeqNum, bool isActive, simtime_t lifeTime);
    IRoute *createRoute(const L3Address& destAddr, const L3Address& nextHop,
            unsigned int hopCount, bool hasValidDestNum,
            unsigned int destSeqNum, bool isActive, simtime_t lifeTime);
    bool updateValidRouteLifeTime(const L3Address& destAddr,
            simtime_t lifetime);
    void scheduleExpungeRoutes();
    void expungeRoutes();

    /*QoSRouting*/
    void recordUtilization(int packetSize,
            Path path = Path(IPv4Address().UNSPECIFIED_ADDRESS,
                    IPv4Address().UNSPECIFIED_ADDRESS));
    void defineBandwidthOverhead(
            std::vector<TransmissionOverHead>& transmissionOverhead,
            double timeSpan);
    void defineRadioBehaviorLoad(
            std::vector<TransmissionTimeAllocation>&transmissionRadioLoad,
            double timeSpan);
    /* Control packet creators */
    AODVQoSRREPACK *createRREPACK();
    AODVQoSRREP *createHelloMessage();
    AODVQoSRREQ *createRREQ(const L3Address& destAddr);
    AODVQoSRREP *createRREP(AODVQoSRREQ *rreq, IRoute *destRoute,
            IRoute *originatorRoute, const L3Address& sourceAddr);
    AODVQoSRREP *createGratuitousRREP(AODVQoSRREQ *rreq,
            IRoute *originatorRoute);
    AODVQoSRERR *createRERR(
    ...
    void handleRREQ(AODVQoSRREQ *rreq, const L3Address& sourceAddr,
    ...
    void sendRREQ(AODVQoSRREQ *rreq, const L3Address& destAddr,
            unsigned int timeToLive);
    void sendRREPACK(AODVQoSRREPACK *rrepACK, const L3Address& destAddr);
    void sendRREP(AODVQoSRREP *rrep, const L3Address& destAddr,
            unsigned int timeToLive);
    void sendGRREP(AODVQoSRREP *grrep, const L3Address& destAddr,
            unsigned int timeToLive);

    /* Control Packet forwarders */
    void forwardRREP(AODVQoSRREP *rrep, const L3Address& destAddr,
            unsigned int timeToLive);
    ...
    virtual Result datagramPreRoutingHook(INetworkDatagram *datagram,
            const InterfaceEntry *inputInterfaceEntry,
            const InterfaceEntry *& outputInterfaceEntry,
            L3Address& nextHopAddress) override
            {
